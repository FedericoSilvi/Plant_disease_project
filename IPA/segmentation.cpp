...
int main()
{
	...
	bool orginalWithNewContour = false;
	std::string nclass = "7a";
	try
	{
		for (int i = 0; i <= 159; i++)
		{
			std::string leafPath = "LEAF CLASS PATH"; // has to be concatened with root path
			...
			for (int k = 0; k < leafCounour.size(); k++)
				cv::drawContours(leafImg, leafCounour, k, cv::Scalar(0, 0, 255), 1, cv::LINE_8);
			...
			// Foreground and background are eroded in order to get granted seeds for each
